...
def main():
    ...
    # asks user if they want to simply run the
    # config file, which is in the same directory
    # as the application
    response = input("Do you want to build from the config file in the current directory (Y/N)? ")
    
    if response == 'Y' or response == 'y':
        # obtains config file path from CURRENT DIRECTORY
        config_path = input("Please enter path of config file: ")
        file = Path(config_path)

        # asks user for desired virtual environment name
        env = input("Please type desired name of virtual environment folder: ")

        # makes projects and/or makes virtual environment
        config_str = parse_config_file(file.absolute())
        create_project(".", env, config_str, True, True)
    else:
        ...
        # makes projects and/or makes virtual environment
        config_str = parse_config_file(config_path)
        create_project(path, venv_path, config_str, make_project, make_venv)
